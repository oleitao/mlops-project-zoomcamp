...
def save_model(model, dv, path="./code/venv/dev/models/model.pkl"):
    """
    Saves the model and DictVectorizer to the specified path using pickle.
    """
    import pickle
...
def send_model_service(model_path):
    """
    Placeholder function for sending the model to a model service.
    Replace this with your actual implementation as needed.
    """
    print(f"Sending model at {model_path} to the model service...")
...
def model_experiment(dataset_path: str="../dataset/churn_train_dataset.csv"):
    ...
    with mlflow.start_run(run_name="model_training") as run:
        ...
        mlflow.set_tag("Developer", "Oleitao")
        ...
        model_file_path = './code/venv/dev/models/model.pkl'
        save_model(model, dv, path=model_file_path)

        # Check file existence before logging
        if not os.path.exists(model_file_path):
            raise FileNotFoundError(f"Model file not found at {model_file_path}")

        # Use absolute path for artifact logging
        abs_model_file_path = os.path.abspath(model_file_path)
        mlflow.log_artifact(abs_model_file_path, artifact_path='preprocessing')

        # Correct way to log the model for registration
    ...
    model_source = abs_model_file_path
